...
int main(void) {
	...
	char c = 'A';
	uint16_t address = 0;
	for (; address < 5; address++)
	{
		byte_write(address, c);
		c++;
	}

	address = 0;
	char s0[6];
	random_read(address, (uint8_t *)s0, 1);
	address++;
	for (; address < 5; address++)
	{
		current_read((uint8_t *)(s0 + address));
	}
	s0[5] = '\0';

	c = 'v';
	address = 4091;
	for (; address <= _24XX32A_MAX_ADDR; address++)
	{
		byte_write(address, c++);
	}
	...
	address = 4091;
	char s1[6];
	random_read(address, (uint8_t *)s1, 1);
	uint8_t index = 1;
	address++;
	for (; address <= _24XX32A_MAX_ADDR; address++)
	{
		current_read((uint8_t *)(s1 + index++));
	}
	s1[5] = '\0';
	
	while(1) {
		printf("Addresses 0 - 4: %s\n", s0);
		printf("Addresses 4091 - 4095: %s\n", s1);
